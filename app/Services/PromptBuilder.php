...
{
    // Extract title from anywhere in the output
    $title = 'New Story';
    if (preg_match('/^Title:\s*(.+)$/mi', $rawOutput, $titleMatch)) {
    ...
    // Strip title, characters block, and any STORY: / TITLE: headers from body
    $body = preg_replace('/^Title:.*$/mi', '', $rawOutput);
    $body = preg_replace('/\[CHARACTERS\].*?\[\/CHARACTERS\]/s', '', $body);
